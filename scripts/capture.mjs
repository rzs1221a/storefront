 ...
 * ── Egress ───────────────────────────────────────────────────────────────
 *
 * This environment's egress proxy only accepts CONNECT-tunnelled traffic.
 * Chromium does not negotiate it and every external request comes back
 * ERR_CONNECTION_RESET — which does not fail loudly, it just yields an
 * unstyled page (the CDN-hosted Tailwind never arrives) and a blank capture.
 *
 * curl traverses the proxy correctly, so external requests are intercepted
 * and served through it. TLS verification stays on throughout: curl uses the
 * CA bundle the environment already configured.
 */

/** Hosts we never fetch — telemetry and analytics only slow the capture down. */
const BLOCKED = [
...
/** Fetch a URL through curl, returning body + content-type. Cached per run. */
async function curlFetch(url) {
  ...
  const promise = (async () => {
    ...
    // Split the trailing metadata line off the binary body.
...
/** Install the curl-backed router on a page. Localhost passes through. */
async function routeThroughCurl(page) {
  await page.route("**/*", async (route) => {
    ...
    try {
       ...
       * iframes (Vimeo backgrounds, Google Maps); when one returns a 403 or a
       * bot-challenge page, the iframe happily renders that HTML — which is
       * how "We couldn't verify the security of your connection" ended up
       * printed across a portfolio screenshot. Aborting leaves the frame
       * blank instead, which is honest and invisible.
       */
      ...
      // A single failed asset should never abort the whole capture; the
      // blankness guard downstream catches the case where it mattered.
      await route.abort();
    }
  });
}
