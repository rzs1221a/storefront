 ...
 *
 * Rendered with Playwright rather than drawn by hand so it uses the real Geist
 * typeface and the real brand tokens — the card and the site cannot drift into
 * looking like two different companies. Reads BRAND, so a rename regenerates
...
const html = `<!doctype html><meta charset="utf-8"><style>
  ...
  * { margin: 0; box-sizing: border-box; }
  ...
  .horizon {
    ...
    background:
      radial-gradient(120% 100% at 78% 100%, rgba(79,214,227,0.16), transparent 62%),
      linear-gradient(180deg, transparent, rgba(79,214,227,0.05));
  }
  .rule {
    position: absolute; left: 0; right: 0; bottom: 38%;
    height: 1px;
    background: linear-gradient(90deg, transparent, rgba(79,214,227,0.45) 30%, rgba(79,214,227,0.12) 70%, transparent);
  }
  ...
  p {
    ...
    color: #b9bcc0; max-width: 30ch; position: relative;
  }
  .foot {
    position: absolute; left: 92px; right: 92px; bottom: 60px;
    display: flex; justify-content: space-between; align-items: baseline;
    font-size: 19px; color: #979da5; letter-spacing: -0.01em;
  }
</style>
...
<div class="brand">
  <svg width="30" height="30" viewBox="0 0 16 16" fill="none">
    <path d="M8 1.2 14.8 8 8 14.8 1.2 8Z" stroke="#4fd6e3"
          stroke-width="1.5" stroke-linejoin="round"/>
    <path d="M8 5.4 10.6 8 8 10.6 5.4 8Z" fill="#4fd6e3"/>
  </svg>
  <span>${BRAND.name}</span>
</div>

<h1>Your website should be the reason they call you.</h1>
